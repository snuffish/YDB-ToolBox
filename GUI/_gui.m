...
FRAME(TITLE,WIDTH,CONTENT)
	N (TITLE,WIDTH,CONTENT,%GPOS)
	S TITLE=$G(TITLE,""),WIDTH=$G(WIDTH,50)

	I $D(CONTENT)#10 S *CONTENT=$$SPLIT^%MPIECE(CONTENT,"\n") ; If a string, split the new-lines into an Array
...
SELECT(LIST)
	...
	I $D(LIST)#10 S *LIST=$$SPLIT^%MPIECE(LIST,"|")
